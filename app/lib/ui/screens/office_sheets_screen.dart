...
class _OfficeSheetsScreenState extends State<OfficeSheetsScreen> {
  ...
  String _value(int r,int c) {
    ...
    final op=RegExp(r'^=([A-H]\d+)\s*([+\-*/])\s*([A-H]\d+)$',caseSensitive:false).firstMatch(raw); if(op==null)return '#FORMULA'; final a=_number(op.group(1)!),b=_number(op.group(3)!); final n=switch(op.group(2)){'+'=>a+b,'-'=>a-b,'*'=>a*b,'/'=>b==0?double.nan:a/b,_=>0}; return n.isNaN?'#DIV/0':_fmt(n);
  }
